...
void ComputerSystem_PowerOn(int argc, char *argv[]) {
  int i, nm;
	...
	// Carga nameFile, el archivo con el nombre del programa a ser ejecutado.
	nameFile = fopen(FILE_PROGRAM_TO_BE_EXECUTED, "r");
	//Si no existe, carga el programa por defecto
	if(nameFile==NULL){
		programFile= fopen(PROGRAM_TO_BE_EXECUTED, "r");
	}
	else{ // Si existe, lee la primera linea
		nameOfFile = fgets(lineRead, LINEMAXIMUMLENGTH, nameFile);
		if(nameOfFile==NULL){ //Si la linea es vacia carga el programa por defecto
			programFile= fopen(PROGRAM_TO_BE_EXECUTED, "r");
		}
		else{ //Si no carga el programa.
			programFile = fopen(nameOfFile,"r");
		}
